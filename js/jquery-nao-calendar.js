...
function($) {
	...
	$.fn.calendar = function (opt,val) {
		...
		def = $.extend(def, opt);
		var weekDays = 'Ned Pon Uto Sre Cet Pet Sub'.split(' ');
		var yearMonths = 'Januar Februar Mart April Maj Jun Jul Avgust Septembar Oktobar Novembar Decembar'.split(' ');
		var yearMonthsShort = 'Jan Feb Mar Apr Maj Jun Jul Avg Sep Okt Nov Dec'.split(' ');
		var today = new Date();
		...
		var yMon = $('<div>')
			...
			.attr('title', 'Izaberite mjesec i godinu!')
			.click(showYearPicker)
		var head = $('<div>')
			.addClass('month-head')
			// .text(yearMonths[date.getMonth()]+' '+date.getFullYear())
			/*.append(btn1) // Izbaceni jer nam ne trebaju
			.append(btn3)
			.append(btn2)
			.append(btn4)*/
			.append(yMon)
